...
def train_model(loader_train, loader_test,model,dann_loss, optimizer,scheduler,hidden_4,epoch,batch_size,parameter,threshold_update=True): ## model training 
    ...
    for i in range(T):  
        ...
        loss = cls_loss + 0.1 * transfer_loss + 0.01 * P_loss + boost_factor * cluster_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
parameter={'hidden_1':128,'hidden_2':64,'num_of_class':2,'cluster_weight':2,'low_rank':32,'upper_threshold':0.9,'lower_threshold':0.5,'boost_type':'linear'}
