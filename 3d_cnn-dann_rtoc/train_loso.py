...
# 🚀 專注於 Valence 與微量分類輔助的實驗 (固定閾值版)
WANDB_PROJECT = "EEG-Arousal-Ablation-threshold"

# ===== 參數設定 =====
NPZ_PATH = r"D:\Deap_eeg\3d_cnn-dann_rtoc\deap_a_minmax.npz" # 確保這裡是 valence 的檔案
OUT_DIR = r"D:\Deap_eeg\3d_cnn-dann_rtoc\Results_LOSO\arousal_0421_threshold"
...
def train_one_loso_fold(test_sub_id):
    ...
    wandb.init(project=WANDB_PROJECT, name=sub_name, group="LOSO_Ablation_Fixed", reinit=True)
    
    log_file = open(os.path.join(sub_dir, f"train_log_{sub_name}.txt"), "a", encoding="utf-8")
    log_file.write(f"\n--- 訓練啟動 {sub_name} (Fixed Threshold) | {time.ctime()} ---\n")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for i, (inputs, labels, subjects) in enumerate(pbar):
            ...
            # 🚀 訓練修改：訓練時的輔助分類標籤，統一改用 0.4375 作為分界
        ...
        with torch.no_grad():
            for inputs, labels, _ in test_loader:
                ...
                # 測試時關閉領域對齊 (alpha=0)
                _, reg_outputs, _, _ = model(inputs, alpha=0)
                
                # 🚀 驗證修改 1：正式改用「迴歸能量值 (reg_outputs)」進行評估
                reg_probs = reg_outputs.squeeze()
                
                if not printed_batch:
                    gt_print = labels[:10].cpu().numpy()
                    pred_print = reg_probs[:10].cpu().numpy() 
                    monitor_str = f"   👉 [盲測監控] GT: {np.round(gt_print, 2)} | Pred(Reg): {np.round(pred_print, 2)}\n"
                ...
                # 🚀 驗證修改 2：真實標籤也嚴格使用 0.4375 來切分出 0 和 1
        ...
        log_str = f"Epoch {epoch+1:02d} | Train MSE: {avg_train_mse:.4f} | Val AUC: {val_auc:.4f} | Val Acc: {val_acc:.4f} | Fixed Thresh: {FIXED_THRESHOLD}\n"
        print(log_str.strip())
...
if __name__ == "__main__":
    ...
    print(f"論文回報格式: {mean_acc:.4f} ± {std_acc:.4f} (Fixed Threshold: 0.4375)")
    print("="*40 + "\n")