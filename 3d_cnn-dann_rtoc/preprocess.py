...
SAVE_PATH = r"D:\Deap_eeg\3d_cnn-dann_rtoc\deap_a_minmax.npz"
...
if __name__ == "__main__":
    ...
    TARGET_TYPE = 0 
    TARGET_NAME = "Valence" if TARGET_TYPE == 0 else "Arousal"
    ...
    for sub_idx in range(1, 33):
        ...
        #target_labels = labels[:, 1].astype(np.float32) # 保留 1~9 分
        target_labels = ((labels[:, 1] - 1.0) / 8.0).astype(np.float32)
        
        sub_X = [] # 暫存這受試者的資料
        for t in range(40):
            de_segs = process_trial_to_de_topo(eeg[t]) # (60, 1, 7, 5, 4)
            sub_X.append(de_segs)
            all_y.append(np.full(60, target_labels[t]))
            all_s.append(np.full(60, sub_idx - 1))
        
        # 受試者內正規化 (Subject-wise Normalization)
        sub_X_np = np.concatenate(sub_X, axis=0) # (2400, 1, 7, 5, 4)
        sub_mean = np.mean(sub_X_np)
        sub_std = np.std(sub_X_np)
        sub_X_np = (sub_X_np - sub_mean) / (sub_std + 1e-6)

        all_X.append(sub_X_np)
        print(f"Sub {sub_idx} processed. Mean: {sub_mean:.2f}, Std: {sub_std:.2f}")
