...
from model_v4 import EEGEmotionNetTopoLG3D, ReverseLayerF
from dataloader_rpli import load_deap_data, sample_groups
from model_PR_PL_v4 import Domain_adaption_model
...
class Classifier(nn.Module):
    def __init__(self, input_dim=128): # 32*3=96 (EEGEmotionNetTopoLG3D 的輸出維度)
...
class DCD(nn.Module):
    ...
    def __init__(self, input_features=256): # 🚀 修改：兩個樣本特徵拼接 128*2 = 256
...
def train_rpli_for_subject(target_id):
    ...
    # 🚀 1. 給情緒分類器用的 Loss (修改：加上 0.1 的標籤平滑，容忍 10% 的受試者主觀打分誤差)
    loss_fn_cls = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.1)
