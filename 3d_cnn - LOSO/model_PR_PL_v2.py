...
class feature_extractor(nn.Module):
    def __init__(self,hidden_1,hidden_2):
         ...
         self.fc1=nn.Linear(140,hidden_1)
         self.bn1=nn.BatchNorm1d(hidden_1)  # 新增第一層 BN
         self.fc2=nn.Linear(hidden_1,hidden_2)
         self.bn2=nn.BatchNorm1d(hidden_2)  # 新增第二層 BN
         self.dropout1 = nn.Dropout(p=0.4)  # 提高 Dropout 比例 (0.25 -> 0.4)
         self.dropout2 = nn.Dropout(p=0.4)  # 提高 Dropout 比例 (0.25 -> 0.4)
         
    def forward(self,x):
         x=self.fc1(x)
         x=self.bn1(x)        # 經過 BN 層
         x=F.relu(x)
         x=self.dropout1(x)   # 提早 Dropout 增加泛化能力
         
         x=self.fc2(x)
         x=self.bn2(x)        # 經過 BN 層
         x=F.relu(x)
         x=self.dropout2(x)
         return x
    ...
    def get_parameters(self) -> List[Dict]:
         ...
         params = [
            ...
            {"params": self.bn1.parameters(), "lr_mult": 1},
            {"params": self.fc2.parameters(), "lr_mult": 1},
            {"params": self.bn2.parameters(), "lr_mult": 1},
                  ]
         return params
         # ================= 請把下面這段補回來 =================
...
#       acc=np.sum(label_smooth(test_predict)==test_labels)/len(test_predict)
   ...
   def cluster_label_update(self,source_features,source_labels):
       ...
       # --- 修正：完美解決 2 類別的空群組與標籤衝突問題 ---
       # 統計每個 cluster 裡面包含的真實標籤數量
       count_0 = np.bincount(source_labels[source_cluster == 0], minlength=self.num_of_class)
       count_1 = np.bincount(source_labels[source_cluster == 1], minlength=self.num_of_class)
       
       # 比較兩種映射組合的正確數，取最大者
       # 組合A：cluster 0 映射到 label 0, cluster 1 映射到 label 1
       score_A = count_0[0] + count_1[1]
       # 組合B：cluster 0 映射到 label 1, cluster 1 映射到 label 0
       score_B = count_0[1] + count_1[0]
       
       if score_A >= score_B:
           self.cluster_label[0] = 0
           self.cluster_label[1] = 1
       else:
           self.cluster_label[0] = 1
           self.cluster_label[1] = 0
