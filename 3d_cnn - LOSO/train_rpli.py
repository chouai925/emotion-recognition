...
# 3. 核心訓練流程 (LOSO 交叉驗證)
# ==========================================
NPZ_PATH = r"C:\chou\Deap_eeg\3d_cnn - LOSO\deap_eeg_de_topo_a.npz"
PRPL_MODEL_DIR = r"C:\chou\Deap_eeg\3d_cnn - LOSO\PR-PL-main\savemodel_early_a" 
SAVE_DIR = r"C:\chou\Deap_eeg\3d_cnn - LOSO\rpli_final_results_A"
os.makedirs(SAVE_DIR, exist_ok=True)
...
def train_rpli_for_subject(target_id):
    ...
    for epoch in range(100):
        ...
        # --- 1. 設定動態權重與置信度門檻 ---
        # 隨著 Epoch 增加，MMD 權重下降，CMMD 權重上升
        # 加入縮放係數，防止 MMD/CMMD 的梯度破壞分類特徵
        # 論文實務上，這些對齊項的權重通常落在 0.01 ~ 0.1 之間
        scale_factor = 0.05  
        
        # 隨著 Epoch 增加，MMD 權重下降，CMMD 權重上升
        alpha = max(0.01, 1.0 - epoch / 100.0) * scale_factor
        beta = min(1.0, epoch / 100.0) * scale_factor
        ...
        for x1, x2, gy in group_loader:
            ...
            src_x, src_y = src_x.to(device), src_y.to(device)
            src_feat = encoder(src_x)
            loss_cls = loss_fn(classifier(src_feat), src_y)
            ...
            # 計算 CMMD (局部特徵條件對齊)
            if confident_tgt_feat.size(0) > 0: # 確保過濾後還有樣本
                loss_cmmd = cmmd(src_feat, confident_tgt_feat, src_y, confident_tgt_pseudo_y)
            else:
                loss_cmmd = torch.tensor(0.0).to(device)
            
            # ==========================================
            # (C) 組合所有 Loss (分類 + 對抗 + MMD + CMMD)
            # ==========================================
            loss_total = loss_cls + (0.05 * loss_adv) + (alpha * loss_mmd) + (beta * loss_cmmd)
            loss_total.backward()
            optimizer_gh.step()
